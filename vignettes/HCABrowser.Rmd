...
hca2 <- hca %>% filter(organ.text %in% c('Brain', 'brain'), 'specimen_from_organism_json.biomaterial_core.ncbi_taxon_id' == 9606)
hca <- hca %>% filter(organ.text %in% c('Brain', 'brain') & 'specimen_from_organism_json.biomaterial_core.ncbi_taxon_id' == 9606)
hca
```
...
```{r select}
hca22 <- hca %>% select('paired_ends', 'manifest.files.name')
