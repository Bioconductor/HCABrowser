...
## The HumanCellAtlas object is activated here by 'files'
...
```{r complexFilter}
hca2 <- hca %>% filter((organ.text %in% c('Brain', 'brain') & !organ.text == 'brain') |
                       !((paired_end != 'False' &
                          specimen_from_organism_json.biomaterial_core.ncbi_taxon_id == 9606)
...
The `HumanCellAtlas` object can undo the most recent queries run on it.
...
to the `HumanCellAtlas` object, the `resetEsQuery()` method can be used.
...
## Obtaining results from the HumanCellAtlas object

Once the user is satisfied with their query, the `downloadHCA()` method can be
...
```{r downloadHCA}
#res <- hca %>% downloadHCA(n = 36)
#res
```

One can also simply pull bundles from the HumanCellAtlas object's results.

```{r pullBundles}
bundle_fqids <- hca %>% pullBundles
bundle_fqids
```
