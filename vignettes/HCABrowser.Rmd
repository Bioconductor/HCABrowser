...
To show which filters are available to query, use the `supportedFilters()` method.

```{r supportedFilters}
hca %>% supportedFilters
```

Availiable values to these fields can be found using the `availiableValues()` method.
If the parameter `fields` is specified, it will display values related to those
field. If not specified, the values for all fields will be specified.
